 ...
 */
class StockRateAxis(centralValue: Double, maxValue: Double, minValue: Double, priceAxis: StockPriceAxis
) : NumberAxis() {
    ...
    override fun autoAdjustRange() {
        ...
        if (plot is ValueAxisPlot) {
            var dataRange: Range? = plot.getDataRange(priceAxis)
    ...
    private fun createTicks2(var1: TextAnchor?, var2: TextAnchor?, var3: Double): MutableList<Tick>? {
        ...
        list.add(NumberTick(range.upperBound, String.format("%.2f%%", ((range.upperBound - centralValue) / centralValue) * 100), var1, var2, var3))
        return list
    }
}
